...
class Enemy(Entity):
    ...
    def __init__(self, name, enemy, images, position, sound, inverted):

        Entity.__init__(self, name, images, inverted)
    ...
    def move(self, fps):
        if (self.path[self.ipath][0] == self.moved[0] and self.path[self.ipath][1] == self.moved[1]):
            ...
            self.rect.x += self.move_step[self.ipath][0] * self.speed/fps*self.constant
            self.rect.y += self.move_step[self.ipath][1] * self.speed/fps*self.constant
